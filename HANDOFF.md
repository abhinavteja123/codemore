...
**Open items:** user's `HeroOverlay.tsx`/`HeroOverlayRoot.tsx` refactor WIP (type-fixed to keep builds green, unwired, uncommitted at the time of writing — verify state before assuming); Marketplace domain verification (TXT record); hosted-scan click-test + real-phone landing pass (user, ~10 min); then the six-phase roadmap above (**executed in full 2026-07-15 — see §0.12**). Known quirk documented in CONTRIBUTING.md: repo-local Windows `node cli.js` runs can die at exit with the libuv `UV_HANDLE_CLOSING` assertion *after* correct output — published package unaffected, trust CI.

---

## 0.12 Update — 2026-07-15 (0.3.0 shipped: telemetry flywheel, 64-rule Python parity, IDE matrix, monolith deleted, mutation baseline, 50-app benchmark)

All six phases of `my-docs/06-opus-plan-prompts.md` executed in one arc (~21 commits, `6923fb5..95f98e8`). Release state below was **verified by probing at write time**, not assumed.

**Release 0.3.0 — live everywhere except the Marketplace:** npm `codemore@0.3.0` published (`npm view` confirms); `main` pushed to `codemore` (nothing ahead); tag `v0.3.0` re-pointed to `95f98e8` locally **and** on the remote; Release run green. The *first* tag push failed its own release self-scan — 3 BLOCKERs, all intentional demo secrets in `src/App.tsx` + `src/components/CodeMoreLinterSandbox.tsx` whose `bdf1748` suppressions were lost in the WebVortexFix (`51e0f99`); `95f98e8` re-adds the scoped `codemore-ignore-file` directives, self-scan back to 0 BLOCKERs. The tool caught its own repo again, correctly. **Marketplace is still 0.2.8** — extension 0.3.0 (`npm run vsce:publish`) is the one distribution step left.

**1. Telemetry flywheel (Track C):** Supabase migration 007 `rule_events` (user applied it in the SQL editor); per-rule verdict fan-out in `/api/telemetry` + `web/src/lib/telemetryVerdicts.ts`; `scripts/telemetry-report.js` (promotion candidates at <5% FP over ≥50 verdicts, demotion at >10%); `.github/workflows/auto-demote-rules.yml` nightly — opens an ISSUE, not a PR; repo secrets `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` added by the user and the **first scheduled run already succeeded**. CONTRIBUTING-RULES.md promotion/demotion procedure rewritten to match. +6 tests.

**2. Python parity — catalog 59 → 64:** five new rules (`vibe-py-cookie-missing-flags`, `vibe-py-cors-wildcard-credentials`, `vibe-py-no-input-validation`, `vibe-py-auth-missing-check`, `vibe-py-auth-bola`) plus Python TP/FP fixtures for `core-security-sql-injection-concat` v1.1.1. The other six priority ports (path-traversal, weak-hash, insecure-deserialization, hardcoded-password, tls-disabled, file-upload-no-validation) were verified **already covered** with py detection + fixtures — no work needed. Corpus 64/64 TP + 64/64 FP.

**3. `docs/ide-matrix.md` (A4):** raw stdio handshake PASS (6 tools, real scan evidence); Claude Code live-connected via `claude mcp add`; Cursor verified (dry-run + synthetic full install); Claude Desktop config written live with `.bak`; **Codex honestly unverified** (binary absent on this machine). The PowerShell `--` trap and the Claude Desktop `cmd /c` wrapper are documented. README MCP section links it. GUI screen-recordings per the doc's checklists remain open.

**4. Monolith migration DONE (§0.8.D's "not deletable" verdict overturned):** `staticAnalyzer.ts` (2,686 LOC) **deleted**; `aiService.ts` slimmed 1,827 → ~950 LOC but **kept** — `generateAiFixForIssue` has live callers in `suggestionEngine.ts` + web `fixSuggestions.ts` and the registry has no equivalent (human decision, honored). Web scans now materialize uploads to a temp dir and run the shared `scanProject()` — web gains cross-file rules for the first time. `test/parity.test.ts` 4/4 byte-identical throughout; unit suite 183 → 167 (16 legacy-only tests deleted with their subjects).

**5. Mutation testing (A7):** StrykerJS scoped to `shared/**` — 12,032 mutants, 157-minute run: **16.72% overall / 35.97% covered-only**. Structurally low *by design*: rule packs are gated by the corpus harness, not mocha — documented in `test/MUTATION-BASELINE.md`, don't panic at the number. The 10 worst survivors were killed with exact-value assertions (scoring sign flip, registry confidence-clamp inversion, suppression regex, SVG-classifier XSS downgrade, sarif `endColumn`, lifecycle NaN); tests 167 → 182. `npm run test:mutation` is a **manual pre-release gate, NOT CI** (157 min).

**6. 50-app benchmark (B5, the study half):** `benchmark/targets.json` (50 public AI-built repos) + resumable `scripts/benchmark.js`; all 50 scanned with 0.3.0. Headline: **40% had ≥1 BLOCKER** (starter 30% / ai-builder 37% / hackathon 80%); 8,330 findings, 133 hand-triaged. Published: `benchmark/REPORT.md`, `docs/benchmark.md`, and the `/docs/benchmark` site page (web build green, 89 pages). `results/` is gitignored — the repo↔vulnerability mapping never enters git. No live secrets found in any scanned repo.

**Next steps, in order:**
1. **Marketplace 0.3.0** — `npm run vsce:publish` (separate from npm; user account step).
2. **Rule calibrations from the benchmark triage** (each needs an fp fixture pair + ruleVersion bump): `vibe-xss-dangerously-set` ~90% FP (vendored shadcn `chart.tsx`); `core-security-innerhtml-assignment` ~95% FP at BLOCKER (escapeHtml wraps, vendor bundles); `vibe-auth-missing-session-check` ~60% FP (`withAuth(...)` wrapper composition missed); `core-security-hardcoded-secret-pattern` placeholder/fixture heuristic + **BUG**: real Anthropic keys `sk-ant-api03-` fall through to the generic OpenAI label (regex `sk-ant-(?:api|admin)-` at `core-security-hardcoded-secret-pattern.ts:74`).
3. **`validate-rule-pr.js` `runCliScan` (execSync) discards stdout** on the Windows libuv exit-crash → ~13 false "CLI scan failed" locally; Linux CI unaffected. Fix: read `err.stdout` on nonzero exit. Same class silently masks fp-dir crashes in `measure-accuracy.js`.
4. **`registryAdapter.detectLanguageForRegistry` is narrower than `projectScanner.detectLanguage`** (`.pem`/`.key`/`.npmrc`/`*.env`) — unify so on-save extension scans get the same secret-file coverage as full scans.
5. **Telemetry still needs the Vercel env** for endpoint writes (carried from §0.10); the 30-day FP clock starts when users opt in.
6. `graphify-out/` is stale (the AiService/StaticAnalyzer god-nodes are gone) — re-run the graphify skill on `daemon/`.
7. GUI verification recordings per `docs/ide-matrix.md` checklists (Claude Code `/mcp`, Cursor settings, Claude Desktop, Codex install).

---

## 0.13 Update — 2026-07-18 (0.3.1 leak fix shipped; 0.3.2 parity + interactive-save ready to publish)

**0.3.1 (published):** Marketplace SecretScan rejected the 0.3.0 VSIX — it bundled `.env.vercel-production` **and** the entire `.stryker-tmp/` mutation sandbox (852 files vs the intended ~24). Worse: the **published 0.2.8 VSIX carried the same env file** — every secret in it (SUPABASE_SERVICE_ROLE_KEY, GITHUB/GOOGLE client secrets, NEXTAUTH_SECRET, CODEMORE_JOB_ENCRYPTION_KEY, CODEMORE_AI_API_KEY) must be **rotated at source** (user notified; verify done before trusting those keys). Root cause: vsce ignores `.gitignore` once `.vscodeignore` exists, and the denylist never covered dotenv files. `224fa1c` hardens `.vscodeignore` (env at every depth, stryker/reports/benchmark, `report.json` — scan reports embed secret-shaped snippets, my-docs, stray exes); `3cdefdb` = 0.3.1. npm was never affected (`files` allowlist).

**0.3.2 (committed local, unpublished):** user field-tested and found the CLI/extension finding-count gap (8 vs 14 on one project) + no way to save a report from the interactive menu. Root causes: extension `analyzeWorkspace` hardcoded `enableExperimental: true` (`daemon/index.ts:357`) and `scanFileWithRegistry` defaulted `?? true`; menu never offered `--out`. Fixes: `ed930ae` (new `codemore.enableExperimental` setting, default false, live-updating — every surface now defaults experimental OFF), `bdc1916` (menu prompts: out-file, json/sarif, experimental toggle; `buildScanArgv` unit-tested), `1fcc9cc` (same class on web `productionAnalyzer` + MCP `validate_fix`/agenticFixer via `validatorHarness` default; experimental auto-on only when the targeted finding is itself experimental; web↔CLI parity test added), `bd5f9c7` (0.3.2 bump). Verified: 184 unit tests, parity 4/4, compile green, self-scan 0 BLOCKERs, VSIX 24 files clean, live 7-vs-8 flag proof, prompts.inject menu runtime proof.

**Publish 0.3.2 (user):** `git push codemore main` → `git tag v0.3.2 && git push codemore v0.3.2` (npm via pipeline) → `npm run vsce:publish` (Marketplace — supersedes leaking 0.2.8; consider unpublishing 0.2.8) → `mcp-publisher.exe publish` if registry version matters. §0.12's "Marketplace 0.3.0" step is superseded — publish 0.3.2 directly.
