...
export default function CodeMoreLinterSandbox() {
  ...
  const [patchedIds, setPatchedIds] = useState<Record<string, boolean>>({});

  const statusColor = (() => {
    if (isScanning) return "border-yellow-500/40 shadow-[0_0_25px_rgba(234,179,8,0.12)]";
    if (!hasScanRun) return "border-gray-900";
    if (findings.length === 0) return "border-emerald-500/40 shadow-[0_0_25px_rgba(16,185,129,0.12)]";
    const hasCritical = findings.some(f => f.type === "CRITICAL");
    if (hasCritical) return "border-red-500/40 shadow-[0_0_25px_rgba(239,68,68,0.12)]";
    return "border-amber-500/40 shadow-[0_0_25px_rgba(245,158,11,0.12)]";
  })();
  ...
  const handleApplyCleanFix = (finding: any) => {
    ...
    setTimeout(() => {
      ...
      // Mark as patched
      setPatchedIds(prev => ({ ...prev, [finding.id]: true }));
      
      // Reset patched state after 1.5 seconds
      setTimeout(() => {
        setPatchedIds(prev => ({ ...prev, [finding.id]: false }));
      }, 1500);
    }, 800);
  };
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className={`relative rounded-xl border bg-[#040409] overflow-hidden flex flex-col h-[420px] transition-all duration-700 ${statusColor}`}>
              ...
              {findings.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                  {findings.map((finding) => (
                    <div 
                      key={finding.id}
                      className={`p-4 rounded-xl border flex flex-col md:flex-row md:items-start justify-between gap-4 transition-all duration-300 animate-slide-in ${
                    ...
                    >
                      ...
                      <button
                        ...
                        disabled={patchingId === finding.id}
                        className={`self-start md:self-center text-xs font-mono rounded-lg px-3.5 py-1.5 whitespace-nowrap transition-all border ${
                          patchedIds[finding.id]
                            ? "bg-emerald-950/20 text-emerald-400 border-emerald-500/30"
                            : "bg-gray-900 hover:bg-gray-800 border-gray-800 text-white"
                        }`}
                      >
                        {patchingId === finding.id ? (
